...
export abstract class DataSets {
    static readonly random: DataSet = {
        name: "Random",
        generate: n => Array.from({length: n}, x => Math.floor(Math.random() * n))
    }
    static readonly sorted: DataSet = {
        name: "Sorted",
        generate: n => Array.from({length: n}, (x, i) => i)
    }
    ...
    static readonly semiSorted: DataSet = {
        name: "Semi Sorted",
        generate: n => Array.from({length: n}, (x, i) => i % 20 < 10 ? i : Math.random() * n)
    }
    ...
    static readonly zigzag: DataSet = {
        name: "Zig Zag",
        generate: n => Array.from({length: n}, (x, i) => i % 2 == 0 ? i : n - i)
    }
    static readonly sawtooth: DataSet = {
        name: "Sawtooth",
        generate: n => Array.from({length: n}, (x, i) => i % Math.sqrt(n))
    }
    static readonly square: DataSet = {
        name: "Square",
        generate: n => Array.from({length: n}, (x, i) => i % Math.sqrt(n) > 0.5 * Math.sqrt(n) ? n : 0)
    }
    static readonly zeroes: DataSet = {
        name: "Zeroes",
        generate: n => Array.from({length: n}, x => 0)
    }
    static readonly thousands: DataSet = {
        name: "Thousands",
        generate: n => Array.from({length: n}, x => Math.floor(Math.random() * 1000))
    }
    static readonly millions: DataSet = {
        name: "Millions",
        generate: n => Array.from({length: n}, x => Math.floor(Math.random() * 1000000))
    }
