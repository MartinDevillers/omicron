...
const prepareAnalysisData = (props:AnalysisSeriesProps) => props.data
    ? props.data
    : analyzer(props.algorithms, props.dataSets, props.sizes, props.scatter)

const AnalysisSeries = (props:AnalysisSeriesProps) => prepareAnalysisData(props).reduce((series, current) => {
    ...
    if (scatterSeries === undefined) {
        ...
        props.showRange && series.push(errorSeries)
    }
    scatterSeries.props.data.push({ x: current.dataSetSize, y: current.actualOperations } as Highcharts.Point)
    // @ts-ignore
    props.showRange && errorSeries.props.data.push({ x: current.dataSetSize, low: current.expectedOperationsBest, high: current.expectedOperationsWorst } as ErrorBarPoint)
    return series;
