...
export default class TimSort extends Algorithm {
     ...
     * small integers.
     */
    static readonly POWERS_OF_TEN = [1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9]
    ...
    static TimSortInner = class {
        ...
        mergeAt(i: number) {
             ...
             */
            let k = this.outer.gallopRight(array[start2], array, start1, length1, 0, compare);
            start1 += k;
            length1 -= k;
     ...
     *     comparator.
     */
    sort(array: number[], compare: Comparer = this.alphabeticalCompare.bind(this), lo: number = 0, hi: number = array.length) {
        ...
        let ts = new TimSort.TimSortInner(array, compare, this);
